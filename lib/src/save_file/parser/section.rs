...
/// A section extracted from a ck3 save file. Not yet parsed.
/// Must be either skipped or parsed.
pub trait SaveFileSection {
...
pub struct TextSection<'tape, R: Read> {
    tape: &'tape mut TextTokenReader<R>,
    name: String,
}

impl<'tape, R: Read> TextSection<'tape, R> {
    pub fn new(tape: &'tape mut TextTokenReader<R>, name: String) -> Self {
        TextSection { tape, name }
    }
}

impl<'tape, R: Read> SaveFileSection for TextSection<'tape, R> {
    fn get_name(&self) -> &str {
        &self.name
    }
...
pub struct BinarySection<'tape, 'resolver, R: Read> {
    tape: &'tape mut BinaryTokenReader<R>,
    name: String,
    token_resolver: &'resolver dyn TokenResolver,
}

impl<'tape, 'resolver, R: Read> BinarySection<'tape, 'resolver, R> {
    pub fn new(
        tape: &'tape mut BinaryTokenReader<R>,
        name: String,
        token_resolver: &'resolver dyn TokenResolver,
...
impl<'tape, 'resolver, R: Read> SaveFileSection for BinarySection<'tape, 'resolver, R> {
...
mod tests {
    ...
    fn test_empty() {
        let section = TextSection {
            tape: &mut TokenReader::from_slice(b""),
    ...
    fn test_mixed_obj() {
        let section = TextSection {
            tape: &mut TokenReader::from_slice(b"a b 1=c 2={d=5}}"),
            name: "mixed_obj".to_string(),
        };
    ...
    fn test_mixed_duplicate_keys() {
        let section = TextSection {
            tape: &mut TokenReader::from_slice(b"a b 1=c 2={d=5} 1={e=6}"),
            name: "test".to_string(),
        };
        let obj = section.parse().unwrap();
        obj.as_array()
    ...
    fn test_rgb() {
        let section = TextSection {
            tape: &mut TokenReader::from_slice(b"color1=rgb { 220 220 220 }"),
            name: "test".to_string(),
        };
        let obj = section.parse().unwrap();
        let rgb = obj
    ...
    fn test_utf8() {
        let section = TextSection {
            tape: &mut TokenReader::from_slice("test=\"Malik al-Muazzam Styrkár\"}".as_bytes()),
            name: "test".to_string(),
        };
        let obj = section.parse().unwrap();
        let utf8 = obj
